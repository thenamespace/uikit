...
function TestApp() {
  ...
  function filterENSNames(list, showSubnames) {
    ...
    if (!showSubnames) {
      return list.filter((item: typeof dummyENSNames[number]) => !item.isSubname);
    } else {
      // Flatten all subnames from all root ENS names
      return list.flatMap((item: typeof dummyENSNames[number]) => item.subnames || []);
    }
  }
  ...
  function MainContent() {
    ...
    const filteredENSNames = filterENSNames(dummyENSNames, showSubnames).filter((ens: ENSItem) => {
      ...
      <main className="ns-main">
        <div className="ns-page">
          <aside className="ns-left">
            <ProfileCard
              bannerUrl={selectedENS ? selectedENS.imageUrl : "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQoFRQjM-wM_nXMA03AGDXgJK3VeX7vtD3ctA&s"}
              avatarUrl={selectedENS ? selectedENS.imageUrl : "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQoFRQjM-wM_nXMA03AGDXgJK3VeX7vtD3ctA&s"}
              name={selectedENS ? selectedENS.name : "thecap.eth"}
              username={selectedENS ? selectedENS.name.replace(".eth", "") : "thecap"}
              ...
              subnames={selectedENS && 'subnames' in selectedENS && Array.isArray(selectedENS.subnames) ? selectedENS.subnames.length : 0}
              profit={6}
              volume={0}
            ...
            <div className="ns-header">
              <div className="ns-tabs">
                ...
                <span className={showSubnames ? "active" : ""} onClick={() => setShowSubnames(true)}>
                  <Text>Subnames</Text>{" "}
                  <Text weight="bold" color="primary">
                    {dummyENSNames.reduce((acc, item) => acc + (item.subnames ? item.subnames.length : 0), 0)}
                  </Text>
                </span>
              </div>
              ...
              {selectedENS && !selectedENS.isSubname && Array.isArray((selectedENS as any).subnames) && (selectedENS as any).subnames.length > 0
                ...
                : filteredENSNames.map((ens: ENSItem, idx: number) => (
                  <ENSNameCard
                    ...
                    expires={ens.expires}
                    chain={ens.chain}
                    {...(!ens.isSubname ? { onClick: () => setSelectedENS(ens) } : {})}
                  />
                ))}
            </div>
          </section>
  ...
  function Sidebar() {
    ...
    return (
      <aside
        className={`ns-sidebar ${expanded ? "expanded" : ""}`}

      >
        <div className="ns-menu">
          <span onClick={() => setExpanded(!expanded)}>
            {!expanded && <div className="ns-logo">N</div>}
            {expanded && <div className="ns-logo">Namespace</div>}
          </span>

          <span>
            <Icon name="person" />
            {expanded && <Text size="md" className="ns-menu-label">Rocket</Text>}
          </span>

          <span>
            <Icon name="search" />
            {expanded && <Text size="md" className="ns-menu-label">Apps</Text>}
          </span>

          <span>
            <Icon name="globe" />
            {expanded && <Text size="md" className="ns-menu-label">Frame</Text>}
          </span>

          <span>
            <Icon name="circle-alert" />
            {expanded && <Text size="md" className="ns-menu-label">Circle</Text>}
          </span>

          <span>
            <Icon name="box" />
            {expanded && <Text size="md" className="ns-menu-label">Box</Text>}
          </span>

          <span>
            <Icon name="square-user" />
            {expanded && <Text size="md" className="ns-menu-label">Lorem Ipsum</Text>}
          </span>

          <span>
            <Icon name="image" />
            {expanded && <Text size="md" className="ns-menu-label">Messages</Text>}
          </span>
        </div>
      </aside>
    );
