...
export const TextRecords = ({ texts, onTextsChanged }: TextRecordsProps) => {
    ...
    return <div className="ns-text-records">
        {texts.length === 0 && <Text>No texts</Text>}
        {texts.map(text => {
            const supportedText = textsMap[text.key];
            return <div key={text.key} className="mb-2 ns-text-record-field d-flex align-items-center">
                <div className="circle-icon d-flex align-items-center justify-content-center">
                    <Icon color="white" size={15} name={(supportedText?.icon || "person") as any}></Icon>
                </div>
                <Text size="sm" weight="medium">{capitalize(text.key)}</Text>
                <Input value={text.value} onChange={(e) => updateTextValue(text.key, e.target.value)} size="md"></Input>
            </div>
        })}
    </div>
}

const TextRecord = ({ key, value, onChange }: {
    key: string
    value: string
